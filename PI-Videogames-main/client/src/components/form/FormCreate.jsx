...
import { postGame, getAllGenres, getVideogames, getAllPlatforms } from "../../actions";
...
function handleDeleteGenres(el) {
  ...
  return (
    ...
    <form onSubmit={(e) =>  handleSubmit(e)} >    
      <div>
        <label>Name:</label>
        <input type="text" value={input.name} name="name" onChange={handleOnChange}/>
        {error.name && ( <span className={s.red}>{error.name}</span> )}
      </div>

      <div>
        <label>Description:</label>
        <textarea type="text" value={input.description} name="description"  onChange={handleOnChange}/>
        {error.description && ( <span  className={s.red}>{error.description}</span> )}
      </div>

      <div>
        <label>Released:</label>
        <input type="date" value={input.released} name="released"  onChange={handleOnChange}/>
        {error.released && ( <span  className={s.red}>{error.released}</span> )}
      </div>

      <div>
        <label>ImageUrl:</label>
        <input type="text" value={input.img} name="img"  onChange={handleOnChange}/>
        {error.img && ( <span  className={s.red}>{error.img}</span> )}
      </div>
 
      <div>
        <label>Rating:</label>
        <input type="number" value={input.rating} name="rating"  onChange={handleOnChange}/>
    ...
    <div>{
      ...
      <div>{
        ...
        {Object.keys(error).length ? (
          <div >
              <input type="submit" disabled name="Enviar" />
          </div>
        ) : (
          <div>
